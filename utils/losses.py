...
class IPSLoss(nn.Module):
    ...
    `p` ∈ (0, 1], one per supervised position. This module:
      ...
      2. applies one of two variance-reduction strategies:
           - "clipped"        : w = min(1/p, τ)
    ...
    def __init__(
        self,
        variant: Literal["clipped", "self_normalized"] = "clipped",
        clip_tau: float = 30.0,
        pad_index: int = 0,
        eps: float = 1e-8,
    ) -> None:
        super().__init__()
        if variant not in ("clipped", "self_normalized"):
            raise ValueError(f"IPSLoss: unknown variant {variant!r}")
