...
and L_C suggested by Implementation Note in §Appe ndix A of the design
    ...
    for start in range(0, len(users), batch_size):
        ...
        if use_economics and use_cost:
            if model.dataset_name == "amazon_beauty":
                ...
                recency = torch.ones(B, V.shape[1], device=device) * 1.0  # FIX §3.9
...
def train_one_epoch(
    ...
    *,
    ...
    n_items: int,
    ml_n_negs: int = 100,
    ml_in_batch_negs: int = 0,           # FIX §3.2
    ml_loader: Optional[MovieLens10MLoader] = None,
    ...
    for batch in train_loader:
        ...
        if model.dataset_name == "amazon_beauty":
            ...
            random_negs = sample_negatives(tgt_flat, n_items=n_items, n_neg=ml_n_negs,
                                     pad_index=pad_index)        # (M, n_neg)
            
            # FIX §3.2: In-batch hard negatives
            M = tgt_flat.shape[0]
            if ml_in_batch_negs > 0:
                in_batch_idx = torch.randperm(M, device=device)[:min(M - 1, ml_in_batch_negs)]
                hard_negs = tgt_flat[in_batch_idx].unsqueeze(0).expand(M, -1)  # (M, K_h)
                cand = torch.cat([tgt_flat.unsqueeze(1), random_negs, hard_negs], dim=1)
            else:
            ...
            if loss_mode == "seq_choice":
                ...
                # FIX §3.3: Add torch.no_grad() to prevent propensity gradient leak
                with torch.no_grad() if loss_mode == "ips_choice" else _nullcontext():
                    p_logit = model.deconfounder.propensity_logit(z_tgt)
                    propensity = torch.sigmoid(p_logit)
...
def main() -> int:
    ...
    if dataset_kind == "amazon_beauty":
        ...
        # FIX §3.4: Use defaultdict and median to prevent dynamic Z overwrite
        per_item_z = defaultdict(list)
        ...
        for iid, zs in per_item_z.items():
            # Median is more robust against extremes
    ...
    ml_in_batch_negs = int(train_cfg.get("ml_in_batch_negs", 63)) # Extracted for in-batch negatives
    grad_clip = float(train_cfg.get("grad_clip", 1.0))
    val_batch_size = int(train_cfg.get("val_batch_size", 256))
    
    # Extract warmup epochs for the new alpha schedule
    ...
    for epoch in range(1, max_epochs + 1):
        ...
        # FIX §3.5: Use alpha_schedule_warmup to ensure adversarial pressure takes effect
        alpha = alpha_schedule_warmup(epoch, grl_alpha_warmup_epochs) if ablation.get("use_grl", True) else 0.0
