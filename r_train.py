...
Post-mortem fixes applied in this revision
------------------------------------------
The "CEINN_訓練收斂困難_根因分析報告" identified ten root causes for the
~30 % (Amazon) and ~57 % (MovieLens) Val-NDCG@10 gap to SASRec. Items
that needed `train.py` changes are wired in here:

* §3.2 — In-batch hard negatives (`ml_in_batch_negs` YAML field).
  Random negatives saturate the 51-way softmax within 5 epochs; in-batch
  negs sit on the user-preferred manifold and resemble the Full-Ranking
  distribution at evaluation time.
* §3.3 — Wrap propensity lookup in `torch.no_grad()` under the main
  `ips_choice` mode for the MovieLens path. The previous code let the
  choice loss backprop into the propensity estimator, which then
  degenerated `1/p̂` to ≈ 1 (i.e. silently disabled IPS correction).
* §3.4 — Per-item Z bucket aggregation: median over all observations of
  an item, not last-observed overwrite. The old loop left `last_z[iid]`
  as an arbitrary near-end-of-training Z, which made PopPress a static
  proxy and gave the discriminator a near-trivial task.
* §3.5 — Switch GRL alpha from `alpha_schedule(epoch / max_epochs)` to
  `alpha_schedule_warmup(epoch − 1, grl_alpha_warmup_epochs)` so the
  adversarial pressure is real (≥ 0.5) by epoch 5 instead of being
  effectively off for every realistic early-stopped run.
* §3.6 — ML inline IPS replaced with the IPSLoss class so the new
  `clipped_self_normalized` variant (gold-standard robust IPS) becomes
  selectable from YAML.
* §3.9 — Validation `recency = 1.0` (was 0.5), aligned with the
  test-time convention.
* Model-side knobs (§3.1 W_V identity init, §3.7 cost auto-scaling) are
  forwarded from YAML through the builder functions; the same scaling
  is applied at validation time so train/val see identical cost
  magnitudes.
...
3. MovieLens uses negative sampling at train time
   ...
   §3.2 fix supplements with in-batch hard negatives.
...
4. Propensity training
   ...
   BEFORE the main loop starts. Afterwards the main `ips_choice` loss
   queries it under `torch.no_grad()` so subsequent updates come ONLY
   from the warmup BCE signal during the next epoch's first batch — in
   practice it stays close to its warmup state.
...
from models.causal_deconfounder import alpha_schedule, alpha_schedule_warmup
...
def sample_in_batch_negatives(
    ...
    """
    In-batch hard negatives (root cause §3.2 fix).

    Random negatives are sampled uniformly from the full item catalogue;
    most are unpopular items the model can trivially distinguish from
    the target. The 51-way training task becomes near-saturated after
    a few epochs while Full-Ranking evaluation still has to defeat
    10k popular items.

    In-batch negatives pull other targets from the same batch as
    additional contrast — these are necessarily items some user actually
    chose at some point, i.e. they sit on the "user-preferred manifold".
    This makes the choice softmax harder and more closely matches the
    distribution the model faces at evaluation time.

    Parameters
    ----------
    positive_targets : (M,) int64 — the batch's true target items.
    n_neg            : desired number of in-batch negatives per row.
                       Will be clamped to ≤ M − 1 (can't use a row's own
                       target as its own negative).

    Returns
    -------
    (M, n_eff) int64 tensor, or None if M ≤ 1 (insufficient batch
    diversity to sample anything useful).
    """
    M = positive_targets.shape[0]
    if M <= 1 or n_neg <= 0:
        return None
    n_eff = min(n_neg, M - 1)
    # Random permutation gives us a fresh draw each step; this matters
    # because hard-neg distribution should not be deterministic.
    idx = torch.randperm(M, device=positive_targets.device)[:n_eff]
    return positive_targets[idx].unsqueeze(0).expand(M, -1)
    ...
    for start in range(0, len(users), batch_size):
        ...
        if use_economics and use_cost:
            if model.dataset_name == "amazon_beauty":
                ...
                # §3.7 fix: apply the same auto-scaling as training so
                # the scoring magnitudes are consistent between train
                # and eval. When cost_scale_mode='none' this is a no-op.
                C_scaled = model.economics._maybe_scale_cost(V, C_all, lam)
                scores = V - lam.unsqueeze(-1) * C_scaled
            else:
                ...
                # RecencyPress: §3.9 fix.
                # At validation time the target IS the most-recent prediction
                # we make for this user (we've consumed the entire training
                # history and we're predicting i_{n+1}). The previous value
                # of 0.5 was an arbitrary middle-ground that did not match
                # either the training-time formula (position_in_seq /
                # seq_len ∈ [0, 1]) or the test-time value (1.0). Use 1.0
                # here so val / test agree, and because β2 · 1.0 is a per-
                # batch constant in any case (it cancels in the softmax),
                # this change is harmless on the indicator metrics but
                # important for consistency-checking ablation A6 (no cost).
                recency = torch.ones(B, V.shape[1], device=device)
                ...
                # §3.7 fix: apply the same auto-scaling as training.
                C_scaled = model.economics._maybe_scale_cost(V, C, lam)
                scores = V - lam.unsqueeze(-1) * C_scaled
...
def train_one_epoch(
    ...
    *,
    ...
    ---------------------------
    `ml_n_negs` uniform-random negatives are always sampled. If
    `ml_in_batch_negs > 0`, an additional set of in-batch hard negatives
    (root cause §3.2 fix) is appended — these come from the batch's own
    target list and sit on the "user-preferred manifold", making the
    softmax task harder and closer to Full-Ranking evaluation.
    ...
    for batch in train_loader:
        ...
        if model.dataset_name == "amazon_beauty":
            ...
            random_negs = sample_negatives(
                tgt_flat, n_items=n_items, n_neg=ml_n_negs, pad_index=pad_index,
            )                                                       # (M, n_neg_random)
            if ml_in_batch_negs > 0:
                # §3.2 fix: in-batch hard negatives. May return None if
                # the batch is degenerately small (M <= 1).
                hard_negs = sample_in_batch_negatives(tgt_flat, ml_in_batch_negs)
                if hard_negs is not None:
                    cand = torch.cat(
                        [tgt_flat.unsqueeze(1), random_negs, hard_negs], dim=1
                    )                                              # (M, 1 + n_random + n_hard)
                else:
                    cand = torch.cat([tgt_flat.unsqueeze(1), random_negs], dim=1)
            else:
                cand = torch.cat([tgt_flat.unsqueeze(1), random_negs], dim=1)  # (M, 1+n_neg)

            # V on the small candidate set: bilinear of h_flat with the
            # candidate embeddings. Handle both bilinear modes:
            #   - 'bilinear' / 'bilinear_xavier' → use W_V (now identity-
            #     initialised by default; see economics_utility.py).
            #   - 'dot' → W_V is None; bypass to plain dot product.
            cand_emb = model.backbone.item_embedding(cand)           # (M, K, d)
            if model.economics.value_head.W_V is None:
                h_proj = h_flat
            else:
            ...
            if use_economics and use_cost:
                ...
                # Honour the YAML-configured cost_scale_mode here too —
                # the ML training step bypasses compute_utility_movielens
                # (because it works on a (M, K) candidate set rather than
                # the (B, V_cat) full catalogue), so we reach into the
                # model's helper to keep the scaling logic in one place
                # (root cause §3.7 fix).
                C_scaled = model.economics._maybe_scale_cost(V_cand, C_cand, lam)
                util = V_cand - lam.unsqueeze(-1) * C_scaled
            elif use_economics and not use_cost:
                util = V_cand
            else:
                util = V_cand
            ...
            if loss_mode == "seq_choice":
                ...
                # §3.3 fix: wrap propensity lookup in no_grad under the
                # 'ips_choice' main mode so L_C does NOT backprop into the
                # propensity estimator. Without this, the IPS weights
                # 1/p̂ degenerate toward 1 within a few epochs (the model
                # learns to raise p̂ rather than to lower the per-position
                # CE), reducing IPS to plain CE.
                z_tgt = item_z_bucket.to(device)[tgt_flat]
                with torch.no_grad() if loss_mode == "ips_choice" else _nullcontext():
                    p_logit = model.deconfounder.propensity_logit(z_tgt)
                    propensity = torch.sigmoid(p_logit)

                # §3.6 fix: use the IPSLoss class so the new
                # `clipped_self_normalized` variant becomes available
                # (the inline code-path here previously only covered
                # 'clipped' and 'self_normalized'). We pass
                # pad_index=-1 because target_col is all zeros — the
                # standard pad_index=0 would silently exclude every
                # supervised position from the loss.
                ml_ips_loss_fn = IPSLoss(
                    variant=ips_variant, clip_tau=ips_clip_tau, pad_index=-1,
                )
                L_C = ml_ips_loss_fn(util, target_col, propensity)
...
def main() -> int:
    ...
    if dataset_kind == "amazon_beauty":
        ...
        # We need a per-item Z bucket. The previous implementation iterated
        # over all (u, pos) rows and OVERWROTE `last_z[iid]` on each visit,
        # leaving the array with the Z at each item's last seen position —
        # i.e. an arbitrary near-end-of-training value rather than a
        # representative statistic. Root cause §3.4 in the analysis report.
        #
        # Fix: collect every observed Z for each item across the train
        # set, then take the MEDIAN. The median is robust to the tail of
        # rare extreme values that occur when an item's popularity changes
        # abruptly within a session.
        print("    Building per-item Z buckets from dynamic Z (median aggregation) …")
        z_per_row, edges = compute_movielens_z_buckets(loader, n_z_buckets)
        n_items = int(loader.vocab["n_items"])
        per_item_z: Dict[int, List[int]] = defaultdict(list)
        for (u, pos), row_idx in loader._row_index.items():
            iid = loader.train_seqs[u][pos][0]
            per_item_z[iid].append(int(z_per_row[row_idx]))
        last_z = np.zeros(n_items + 1, dtype=np.int64)
        for iid, zs in per_item_z.items():
            # np.median returns a float; cast back to int for the bucket id.
            last_z[iid] = int(np.median(zs))
    ...
    # Post-mortem fixes (§3.1 W_V init, §3.7 cost scale): three architecture
    # knobs forwarded to EconomicsUtility. Defaults match the new safer
    # behaviour (identity init for W_V; no auto-scaling unless asked).
    value_mode               = arch.get("value_mode", "bilinear")
    cost_scale_mode          = arch.get("cost_scale_mode", "none")
    cost_scale_target_ratio  = float(arch.get("cost_scale_target_ratio", 1.0))
    if dataset_kind == "amazon_beauty":
    ...
    print(f"           value_mode={value_mode}, cost_scale_mode={cost_scale_mode}"
          + (f" (target_ratio={cost_scale_target_ratio})" if cost_scale_mode == "auto" else ""))
    ...
    # §3.2 fix: in-batch hard negatives. 0 disables (default for back
    # compat); recommended value for ML is ≈ batch_size - 1 so the
    # training task sees a hard-neg distribution close to the
    # evaluation-time Full-Ranking distribution.
    ml_in_batch_negs = int(train_cfg.get("ml_in_batch_negs", 0))
    ...
    # §3.5 fix: decouple GRL alpha schedule from max_epochs. With the old
    # `alpha_schedule(epoch / max_epochs)` and max_epochs=300 + early-stop at
    # 30 epochs, alpha never exceeded ~0.46. The warmup parametrisation
    # ramps alpha to ~1 over `grl_alpha_warmup_epochs` regardless of the
    # total epoch budget. Default 20 (specified via YAML).
    grl_alpha_warmup_epochs = int(train_cfg.get("grl_alpha_warmup_epochs", 20))
    for epoch in range(1, max_epochs + 1):
        t0 = time.time()
        if ablation.get("use_grl", True):
            # epoch-1 → epoch=1 starts with alpha=0 (no GRL on the very
            # first pass), reaching ~1 at epoch=warmup_epochs+1.
            alpha = alpha_schedule_warmup(epoch - 1, grl_alpha_warmup_epochs)
        else:
            alpha = 0.0
        ...
        step = train_one_epoch(
            ...
            n_items=n_items,
            ml_n_negs=ml_n_negs,
            ml_in_batch_negs=ml_in_batch_negs,
            ml_loader=ml_loader,
