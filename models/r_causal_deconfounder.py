...
def alpha_schedule_warmup(epoch: int, warmup_epochs: int) -> float:
    """
    Sigmoid warm-up that COMPLETES over a fixed `warmup_epochs` rather
    than over the full `max_epochs`.

    Background (root cause §3.5)
    ----------------------------
    The original `alpha_schedule(p)` is parameterised by p = epoch /
    max_epochs. With `max_epochs = 300` and early stopping at epoch 30,
    p never exceeds 0.1 and α stays ≤ 0.46 — i.e. the GRL is effectively
    off for the entire training run. Setting `warmup_epochs ≈ 20`
    decouples the schedule from the training-budget cap and lets the
    adversarial pressure reach full strength inside a realistic
    early-stop window.

    Usage in `train.py`
    -------------------
        alpha = alpha_schedule_warmup(epoch, warmup_epochs=20)
        out = model(batch, grl_alpha=alpha)

    NOTE: the `train.py` currently in the repo still calls
    `alpha_schedule(epoch / max_epochs)`. To use this helper, also
    change the call site there. The YAML field
    `grl_alpha_warmup_epochs` has been added to both configs to
    serve as the input value.
    """
    p = min(1.0, max(0.0, float(epoch) / float(max(1, warmup_epochs))))
    return alpha_schedule(p)
