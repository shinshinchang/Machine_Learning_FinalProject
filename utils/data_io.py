...
"""
def extract_leaf_categories(categories_field: Any) -> List[str]:
    if not isinstance(categories_field, list):
        return []
    leaves: List[str] = []
    for path in categories_field:
...
# 應修正為（僅回傳一個 leaf，對齊 EDA 的 max(cats, key=len) 邏輯）
...
# =============================================================================
# Phase-2 conveniences for the Amazon item-meta pipeline
# =============================================================================

def extract_primary_leaf_category(categories_field: Any) -> str | None:
    """
    Pick a single representative leaf category for an item.

    Plan §2.2.1 requires "one leaf per item" so we can build a 222-way
    lookup table. But the raw `categories` field may contain multiple
    paths (e.g., when a product is cross-listed). We adopt the
    McAuley-dump convention: the FIRST path is the primary classification
    and all later paths are secondary. We therefore take the leaf of the
    first path.

    Returns None if no usable category is present.
    """
    leaves = extract_leaf_categories(categories_field)
    return leaves[0] if leaves else None


def get_price_or_none(price_field: Any) -> float | None:
    """Return the price as a float iff it is a positive finite number."""
    if not has_valid_price(price_field):
        return None
    return float(price_field)


def get_brand_or_none(brand_field: Any) -> str | None:
    """Return the brand string stripped iff non-empty; else None."""
    if not has_valid_brand(brand_field):
        return None
    return str(brand_field).strip()
