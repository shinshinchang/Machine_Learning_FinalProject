...
This module implements the five non-causal baselines that share the
...
Five baselines provided here:
    ...
    SASRecModel   — Self-Attentive Sequential Recommender (Kang & McAuley,
                    2018). Causal Transformer encoder, same shift-by-one
                    full-vocab CE training as GRU4Rec; defined in
                    `models/sasrec.py` and re-exported below.

    BERT4RecModel — Bidirectional Transformer with MLM (Cloze) training
...
# Re-export the Transformer baselines so train_baseline.py can keep using
...
def build_baseline(
    ...
    *,
    ...
    dropout: float = 0.1,
    pad_index: int = 0,
    # Transformer-only kwargs (kw-only so existing positional callers don't break).
    n_heads: int = 2,
    max_seq_len: int = 50,
    ff_mult: int = 4,
) -> nn.Module:
    ...
    Notes
    -----
    * PopRec is created here as a "blank" instance — its real state is
      set by `fit_from_train_seqs` in the training script. Callers
      should treat the returned PopRec as a typed placeholder.
