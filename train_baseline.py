...
-----
    ...
    # SASRec on Amazon (seed 2)
    python train_baseline.py \\
        --config configs/amazon_beauty.yaml \\
        --baseline sasrec --seed 2

    # BERT4Rec on Amazon (seed 0), override mask prob
    python train_baseline.py \\
        --config configs/amazon_beauty.yaml \\
...
* SASRec shares the GRU4Rec training step verbatim (`train_seq_lm_one_epoch`):
  both models expose `forward_full_sequence(item_ids) -> (B, T, V+1)`
  and use shift-by-one full-vocab CE. The only difference is the
  backbone class.

* BERT4Rec uses MLM training: a fresh random mask is drawn per batch
...
def train_sasrec_one_epoch(
    ...
    *,
    ...
    """
    SASRec uses the same shift-by-one full-vocab CE objective as GRU4Rec:
    both models expose `forward_full_sequence(item_ids) -> (B, T, V+1)`.
    The body is intentionally a near-duplicate of `train_gru4rec_one_epoch`
    — kept as a separate function so per-baseline knobs (default grad_clip
    in particular: SASRec papers recommend a looser 5.0 vs GRU4Rec's 1.0)
    live next to the baseline they belong to.
    """
    model.train()
    total_loss = 0.0
    n_steps = 0
...
def random_mask_batch(
    item_ids: torch.Tensor,        # (B, T) long — original sequence (no shift)
    mask_prob: float,
    mask_token_id: int,
    pad_index: int = 0,
) -> Tuple[torch.Tensor, torch.Tensor]:
    """
    Apply BERT-style random masking to a batch of item sequences.

    Each non-PAD position is independently replaced by [MASK] with
    probability `mask_prob`. The target tensor records the ORIGINAL item
    at masked positions and PAD elsewhere — so a single
    `F.cross_entropy(... ignore_index=pad_index)` on `(masked_seq → logits,
    targets)` recovers the standard MLM loss (only masked positions
    contribute).

    Edge-case guard: any row that ends up with zero masked positions (and
    has at least one real item) gets ONE random non-PAD position forced
    to be masked. Without this, that row would consume compute without
    contributing to the loss, and degenerate small batches could yield
    no gradient at all.

    Note (no 80/10/10 trick): BERT's original recipe sometimes replaces
    "to-be-predicted" tokens with random items or keeps them unchanged,
    instead of always replacing them with [MASK]. The BERT4Rec literature
    (and RecBole / pmixer reference implementations) skip this — pure
    [MASK] replacement, no random/unchanged variants. We follow them.

    Returns
    -------
    masked_seq : (B, T) — input with [MASK] at masked positions
    targets    : (B, T) — original item id at masked positions, `pad_index` elsewhere
    """
    device = item_ids.device
    is_pad = item_ids == pad_index
    rand = torch.rand(item_ids.shape, device=device)
    mask_mat = (rand < mask_prob) & (~is_pad)            # (B, T) — True where we mask

    has_real = (~is_pad).any(dim=1)                      # (B,)
    no_mask_rows = (~mask_mat.any(dim=1)) & has_real     # (B,)
    if no_mask_rows.any():
        # For rows that drew zero masks: pick one random non-PAD position.
        # Random score for non-PAD positions, -inf for PAD, take argmax.
        scores = torch.where(
            is_pad,
            torch.full_like(rand, float("-inf")),
            torch.rand(item_ids.shape, device=device),
        )
        pick = scores.argmax(dim=1)                      # (B,)
        rows = torch.nonzero(no_mask_rows, as_tuple=False).squeeze(-1)
        mask_mat[rows, pick[rows]] = True

    masked_seq = torch.where(
        mask_mat,
        torch.full_like(item_ids, mask_token_id),
        item_ids,
    )
    targets = torch.where(
        mask_mat,
        item_ids,
        torch.full_like(item_ids, pad_index),
    )
    return masked_seq, targets


def train_bert4rec_one_epoch(
    model: BERT4RecModel,
    loader: DataLoader,
    optim: torch.optim.Optimizer,
    *,
    ...
    """
    MLM training step for BERT4Rec.

    For each batch we redraw the random mask on-the-fly (so each epoch
    sees fresh corruption of the same sequences — important for
    BERT4Rec's data efficiency claims). The loss is sparse CE: only the
    masked positions contribute, thanks to `ignore_index=pad_index`.

    Note we use `batch.item_ids` directly (the un-shifted input sequence),
    NOT `batch.target_ids`. The standard SequentialNextItemDataset returns
    `target_ids = item_ids` shifted-by-one, which is the supervision
    signal for autoregressive models. BERT4Rec's supervision is
    DERIVED from the input via masking, so it ignores `target_ids`.
    """
    model.train()
    total_loss = 0.0
    n_steps = 0
    ...
    for batch in loader:
        item_ids = batch.item_ids.to(device)         # (B, T)
        masked_seq, targets = random_mask_batch(
            item_ids, mask_prob=mask_prob,
            mask_token_id=mask_token_id, pad_index=pad_index,
        )

        logits = model.forward_masked(masked_seq)        # (B, T, vocab_size)
        B, T, V = logits.shape
        loss = F.cross_entropy(
            logits.reshape(B * T, V),
            targets.reshape(B * T),
...
def run_sasrec(
    *,
    ...
    """
    Train SASRec with full-vocab cross-entropy (shift-by-one supervision).
    Shares the data pipeline with GRU4Rec; only the model class and a few
    Transformer-specific hparams differ.
    """
    d = int(train_cfg.get("d", 64))
    n_layers = int(train_cfg.get("n_layers", 2))
    n_heads = int(train_cfg.get("n_heads", 2))
    dropout = float(train_cfg.get("dropout", 0.2))
    ff_mult = int(train_cfg.get("ff_mult", 4))
    lr = float(train_cfg.get("lr", 5e-4))
    batch_size = int(train_cfg.get("batch_size", 256))
    max_epochs = int(train_cfg.get("max_epochs", 200))
    patience = int(train_cfg.get("early_stop_patience", 10))
    ...
    n_params = sum(p.numel() for p in model.parameters() if p.requires_grad)
    print(f"    Model: SASRec, n_items={n_items}, d={d}, heads={n_heads}, "
          f"layers={n_layers}, dropout={dropout}, ff_mult={ff_mult}")
    print(f"    Trainable params: {n_params:,}")
    ...
    for epoch in range(1, max_epochs + 1):
        ...
        save_checkpoint(out_dir / "last.ckpt", model, extra={
            "epoch": epoch, "val_ndcg10": ndcg10, "baseline": "sasrec",
        })
        if ndcg10 > best_val_ndcg10:
            best_val_ndcg10 = ndcg10
            best_epoch = epoch
...
def run_bert4rec(
    *,
    ...
    """
    Train BERT4Rec with Masked-LM (Cloze) objective.

    Data pipeline is shared with GRU4Rec/SASRec — the same
    `SequentialNextItemDataset` produces (item_ids, target_ids), and
    we just ignore `target_ids` because the supervision is derived
    from `item_ids` via random masking each batch.
    """
    d = int(train_cfg.get("d", 64))
    n_layers = int(train_cfg.get("n_layers", 2))
    n_heads = int(train_cfg.get("n_heads", 2))
    dropout = float(train_cfg.get("dropout", 0.2))
    ff_mult = int(train_cfg.get("ff_mult", 4))
    mask_prob = float(train_cfg.get("mask_prob", 0.2))
    lr = float(train_cfg.get("lr", 5e-4))
    batch_size = int(train_cfg.get("batch_size", 256))
    max_epochs = int(train_cfg.get("max_epochs", 200))
    patience = int(train_cfg.get("early_stop_patience", 10))
    ...
    n_params = sum(p.numel() for p in model.parameters() if p.requires_grad)
    print(f"    Model: BERT4Rec, n_items={n_items}, d={d}, heads={n_heads}, "
          f"layers={n_layers}, dropout={dropout}, ff_mult={ff_mult}, "
          f"mask_prob={mask_prob}")
    print(f"    Trainable params: {n_params:,}  "
          f"(includes [MASK] embedding at vocab idx {model.mask_token_id})")
    ...
    for epoch in range(1, max_epochs + 1):
        t0 = time.time()
        stats = train_bert4rec_one_epoch(
            model, train_loader, optim, device=device,
            mask_prob=mask_prob, pad_index=pad_index,
            grad_clip=grad_clip,
        )
        val_metrics, _ranks, _items = validate_full_ranking(
            model, loader.val_seqs, loader.train_seqs, seen_items,
        ...
        print(f"  Epoch {epoch:3d}/{max_epochs}  L_MLM={stats.loss:.4f}  "
              f"val_NDCG@10={ndcg10:.4f}  ({epoch_time:.1f}s)")

        save_checkpoint(out_dir / "last.ckpt", model, extra={
            "epoch": epoch, "val_ndcg10": ndcg10, "baseline": "bert4rec",
        })
        if ndcg10 > best_val_ndcg10:
            best_val_ndcg10 = ndcg10
            best_epoch = epoch
...
def main() -> int:
    ...
    elif args.baseline == "gru4rec":
        model, best_val_ndcg10, best_epoch = run_gru4rec(
            loader=loader, train_cfg=train_cfg, seen_items=seen_items,
            out_dir=out_dir, device=device, n_items=n_items,
            pad_index=pad_index, max_seq_len=max_seq_len, csv_path=csv_path,
        )
    elif args.baseline == "sasrec":
        model, best_val_ndcg10, best_epoch = run_sasrec(
            loader=loader, train_cfg=train_cfg, seen_items=seen_items,
            out_dir=out_dir, device=device, n_items=n_items,
            pad_index=pad_index, max_seq_len=max_seq_len, csv_path=csv_path,
        )
