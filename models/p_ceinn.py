...
def build_ceinn_movielens(
    ...
    *,
    ...
    MovieLens has NO static Z bucket in vocab_sizes — the caller picks
    a discretisation of the dynamic Z(t) values. `n_z_buckets` defaults
    to 10 (matching Amazon's choice for symmetry).

    The cost backend (MovieLensCost) has no per-item buffers — the
    GenreRed / RecencyPress / PopPress signals are supplied at
    forward-time by the training loop from the loader's precomputed
    pickles.
    """
