...
#
# Plan §5.3: 5 seeds per baseline per dataset. With 5 baselines (poprec,
# bpr_mf, gru4rec, sasrec, bert4rec) and 2 datasets this is 50 runs.
# PopRec runs are essentially instant (no training); BPR-MF and GRU4Rec
# are dominated by data-loading and a single hidden state per step; the
# Transformers (SASRec / BERT4Rec) scale with sequence length squared and
# are by far the longest. Budget accordingly.
#
# Usage
# -----
...
#
#     # Transformer baselines only (skip pop / BPR / GRU)
#     BASELINES="sasrec bert4rec" bash scripts/run_baselines.sh
#
# Environment variables:
#     BASELINES   space-separated list  (default: "poprec bpr_mf gru4rec sasrec bert4rec")
...
BASELINES="${BASELINES:-poprec bpr_mf gru4rec sasrec bert4rec}"
